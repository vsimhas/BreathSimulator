...
volatile BOOT_TRACE_NOINIT uint32_t boot_stage;
volatile BOOT_TRACE_NOINIT uint32_t boot_trace_stage;
volatile BOOT_TRACE_NOINIT uint32_t boot_trace_sync;
volatile BOOT_TRACE_NOINIT uint32_t boot_sync_step;

void BootTraceWrite(uint32_t marker)
{
  boot_trace_stage = marker;
...
#error Please #define CORE_CM4 or CORE_CM7
  ...
  boot_stage = 3U;
  boot_sync_step = 2U;
  BootTraceWrite(BOOT_TRACE_CM7_BASE | 3U);
#endif
}
...
void ExitRun0Mode(void)
{
  boot_stage = 1U;
#if defined(CORE_CM7)
  BootTraceWrite(BOOT_TRACE_CM7_BASE | 1U);
#elif defined(CORE_CM4)
  BootTraceWrite(BOOT_TRACE_CM4_BASE | 1U);
#endif
  boot_sync_step = 1U;
  ...
  #endif /* SMPS */
  /* Wait till voltage level flag is set (bounded — avoid infinite hang at reset). */
  {
    uint32_t pwr_wait = 0x00100000U;
    while (((PWR->CSR1 & PWR_CSR1_ACTVOSRDY) == 0U) && (pwr_wait-- > 0U))
    {
    }
  }
  ...
  /* Exit Run* mode */
  PWR->CR3 &= ~(PWR_CR3_LDOEN);
  /* Wait till voltage level flag is set (bounded — unbounded wait hangs debug at Reset_Handler). */
  {
    uint32_t pwr_wait = 0x00100000U;
    while (((PWR->CSR1 & PWR_CSR1_ACTVOSRDY) == 0U) && (pwr_wait-- > 0U))
    {
  ...
  boot_stage = 2U;
#if defined(CORE_CM7)
  BootTraceWrite(BOOT_TRACE_CM7_BASE | 2U);
#elif defined(CORE_CM4)
  BootTraceWrite(BOOT_TRACE_CM4_BASE | 2U);
#endif
}

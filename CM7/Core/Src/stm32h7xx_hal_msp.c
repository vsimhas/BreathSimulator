...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* hltdc)
{
  ...
  if(hltdc->Instance==LTDC)
  {
    ...
    /* TM043NDH02 typ. 9 MHz pixel clock: 25 MHz / 25 * 144 / 16 = 9 MHz */
    ...
    /* PLL3 / HSE need extra settle time on cold power-up before LTDC starts. */
    {
      uint32_t pll3_wait = 0x00100000U;
      while ((__HAL_RCC_GET_FLAG(RCC_FLAG_PLL3RDY) == RESET) && (pll3_wait-- > 0U))
      {
      }
      HAL_Delay(2);
    }
...
void HAL_SD_MspInit(SD_HandleTypeDef* hsd)
{
  ...
  if(hsd->Instance==SDMMC1)
  {
    ...
    */
    ...
    {
      GPIO_InitTypeDef sd_pull = {0};

      sd_pull.Mode = GPIO_MODE_AF_PP;
      sd_pull.Pull = GPIO_PULLUP;
      sd_pull.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
      sd_pull.Alternate = GPIO_AF12_SDIO1;

      sd_pull.Pin = GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12;
      HAL_GPIO_Init(GPIOC, &sd_pull);

      sd_pull.Pin = GPIO_PIN_2;
      HAL_GPIO_Init(GPIOD, &sd_pull);
    }
